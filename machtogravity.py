...
def datainterp(datapath,intersize,p,npoints):
	...
	interdataname = mainpath+"\\interforce%s.dat"%(int(number))
	#read the file 
	result=pd.read_table(dataname,skiprows=3,names=['1','2','3','4','5','6','7'],sep='  ',iterator=True)
	loop = True
	chunkSize = 100000
	chunks = []
	while loop:
...
def machmain(grid,mach):
	...
	print '#!/bin/bash'
	for i in xrange(9):
		output = ['./trtNoLinear_10',str(grid),str(grid),str(int(gap[i])),str(20),'0',str(("%.14f" %result[i])),str(mu),'1','2']
		print '\t'.join(output)
	return 0
...
if __name__ == '__main__':
	a = r'D:\datapaper\blocktrt\NoLinear\volume\mach\0.02\20to11'
	main(a)
